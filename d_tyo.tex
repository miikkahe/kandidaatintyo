...
hallita. Jälkimmäisiä on selkeintä kutsua hallitsemattomiksi tekijöiksi
...
\item Merkittävien hallittavien tekijöiden \textit{x} asettaminen siten, että
	hallitsemattomien tekijöiden \textit{z} vaikutus on mahdollisimman pieni
\end{itemize}
...
\begin{figure*}
  \begin{center}
    ...
    \subfigure[Prosessin tekijät jaettuna hallittaviin ja hallitsemattomiin.]{
...
ja jotka antavat tietoa myös tekijöiden yhteisvaikutuksista.
...
kohtuullisella koemäärällä. Pienempi koemäärä johtaa tekijöiden vaikutusten
\textit{sulautumiseen}
...
Usein hyödyllisin suunnitelma on \(2^\textit{k-p}\)-koe,
mikä tarkoittaa että koe on \(1/2^\textit{p}\)-osa \(2^\textit{k}\)-kokeesta.
...
ja siinä on 16 koeajoa. Yleensä seulontakokeeksi valitaan
...
sen itse esimerkiksi python tai R -ohjelmointikielillä.
...
\todo{vastepinta, d-optimal, designated design, Box-Meyers}
...
Vasteen satunnaista vaihtelua sanotaan \em varianssiksi, \em
...
\item Kokeeseen pakotetaan varianssia sisällyttämällä siihen
    myös hallitsemattomia tekijöitä;
\item Dispersiovaikutuksia analysoidaan kokeista joissa ei
    ole toistoa. \parencite{Bursztyn}
\end{enumerate}

Hallitsemattomien tekijöiden sisällyttäminen oli japanilaisen
...
Taguchi esitti että vasteen varianssin aiheuttavat usein satunnaiset vaihtelut
...
Taguchi nimesi nämä hallitsemattomiksi tekijöiksi (noise factor)
ja erotti ne hallittavista tekijöistä antaen niille kokeessa erityistä
huomiota. \textcite{Bursztyn} antavat yksinkertaisen esimerkin hallitsemattomien tekijöiden
sisällyttämisestä: Ajatellaan että koe tehdään tuotteelle jonka
komponenttiosan leveys on 3 \pm 0,1 mm.
...
saattaisi olla 2,9 mm, joittenkin 3,1 mm, mutta useimmat olisivat lähellä
3,0 mm. Taguchin menetelmässä komponentin leveys valittaisiin
hallitsemattomaksi tekijäksi ja kokeessa käytettäisiin vain niitä joiden
leveys on 2,9 mm tai 3,1 mm. Koesuunnitelmaan sisältyisi tällöin
kaksitasoinen hallitsematon tekijä jonka $-$-taso on 2,9 mm ja
+-taso 3,1 mm. \todo{regression slope kuvaaja}.
\textcite{steinberg1994,Steinberg1998,Berube1998} ovat osoittaneet
että hallitsemattomien tekijöiden sisällyttäminen kokeeseen voi
...
tai kallista hallita kokeen aikana.

Taguchin menetelmässä hallittaville ja hallitsemattomille
tekijöille valitaan omat erilliset koesuunnitelmansa.
Hallittavien tekijöiden suunnitelmaa nimitetään \em sisätaulukoksi \em
ja hallitsemattomien tekijöiden suunnitelmaa \em ulkotaulukoksi \em
...
Esimerkiksi hallittavien ja hallitsemattomien tekijöiden nimeäminen
...
(engl. response surface methodology, RSM, josta lisää tuonnempana)
...
perinteisen koesuunnittelun eroista \todo
...
sähköpostissa havainneensa Taguchi-menetelmien toimivan hyvin.
...
\item Menetelmä ei kerro tarkalleen kuinka hallitsemattomat tekijät vaikuttavat vasteeseen.
      Syntyykö kohina vain joistain hallitsemattomista tekijöistä? Onko niitten ja
...
tehokkuudesta robustissa suunnittelussa. (Yhdistetyssä suunnitelmassa hallitsemattomia
tekijöitä ei eroteta omaksi ulkotaulukokseen, vaan niitä vaihdellaan yhdessä hallittavien
tekijöiden kanssa kuten normaalissa \(2^\textit{k}\)-kokeessa).
Teoreettiset perustelut yhdistettyjen suunnitelmien paremmuudelle ovat
...
seulomiseksi. He päätyvät siihen tulokseen että hallitsemattomien tekijöiden
...
\cite{frey2008adaptive} ovat tutkineet adaptive OFAT (mukautuva OFAT) -menetelmän
toimivuutta robustissa suunnittelussa. Menetelmä on kuten edellä mainittu
aOFAT, erotuksena se että joka hallittavan tekijän tason vaihdon
jälkeen ajetaan hallitsemattomille tekijöille resoluutio III -tasoinen
...
käytettiin tilastollisen koesuunnittelun menetelmiä jotka
toteutettiin Minitab–ohjelmistolla. Projektissa käytettäväksi
valmisteltiin yksi suuri erä ruiskupinnoitettuja näytekappaleita
sekä kappaleita erilaisilla pinnoitteilla.

Mittausjärjestelmää voidaan ajatella prosessina johon kuuluu
näytteen valmistelu, abraasiotestin toteuttaminen ja massahäviön
mittaaminen. Prosessissa on sekä hallittavia parametreja, tässä mm. painava
...
mahdollisimman pientä. Koesuunnittelun hallittavat ja hallitsemattomat
tekijät vastaavat näitä hallittavia ja hallitsemattomia parametreja joten
