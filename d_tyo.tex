...
\textit{x,x,...} vaikuttavat vasteeseen \textit{y}. Joissain tapauksissa on hyödyllisempää
...
hallita \ref{fig:malli2}. Jälkimmäisiä kutsutaan häiriö- tai kohinatekijöiksi (engl. noise
factor). Vaste \textit{y} ....
On yleistä nimetä tutkittavat tekijät mallilla A, B, C, jne.
\textcite[pp. 3]{Montgomery2012} antaa kokeen
tavoitteeksi seuraavat:

\begin{itemize}
\item Vasteeseen y eniten vaikuttavien tekijöiden tunnistaminen
\item Merkittävien tekijöiden asettaminen siten, että y on mahdollisimman usein
	mahdollisimman lähellä haluttua arvoa
\item Merkittävien tekijöiden asettaminen siten, että y:n vaihtelu on pientä
\item Merkittävien hallittavien tekijöiden x asettaminen siten, että
...
2 tasoa ja 1 tekijällä 3 tasoa. Tällainen koe on \todo suomeksi mixed-level design.
...
B korkeaa tasoa kirjaimella \textit{b}, jne. Merkintä \textit{ac} on siis
Tekjät A ja C korkealla tasolla ja tekijä B matalalla. Esimerkkinä
suunnitelmamatriisista kuva \todo . 

Yksinkertaisinta on muuttaa yhtä tekijää kerrallaan. Koesuunnittelu kuitenkin syntyi
osaksi juuri tarjoamaan vaihtoehtoja tälle menetelmälle. On mahdollista luoda
koesuunnitelmia jotka vaativat vähemmän koeajoja kuin yksi tekijä kerrallaan
-menetelmä, ja jotka antavat tietoa myös tekijöiden yhteisvaikutuksista.
...
tekijöillä on 2 tasoa, on koeajojen määrä 2^\textit{k}, missä \textit{k} on
tekijöiden lukumäärä. Ko. kokeelle käytetäänkin nimenä yksinkertaisesti
merkintää 2^\textit{k} -koe. Jos tekijöitä tiedetään olevan 2, merkintä on
...
kolmen tekijän yhteisvaikutus jne. Suunnitelmissa monen tekijän yhteisvaikutukset
...
kertoo kuinka monet vaikutukset suunnitelmassa on sulautettu. Suuri resoluutio



\todo
Aliasointi! Kaikissa käytännön koesuunnitelmissa



On mahdollista valita tekijöille eri määrä tasoja samassa kokeessa, tällaist
...
\section{Dispersiovaikutukset, robusti suunnittelu ja laadun hallinta}
